			...
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-6">
						<div class="form-group input-group">
							<label>From::</label>
							<input type="text" data-date="" data-date-format="yyyy-mm-dd" data-link-field="any" data-link-format="yyyy-mm-dd" name="date_pickerfrom" id="date_pickerfrom" value="<?php echo isset($_POST['date_pickerfrom']) ? $_POST['date_pickerfrom'] : ''; ?>" readonly="true" class="date_pickerfrom input-sm form-control">
							<span class="input-group-btn">
								<i class="fa  fa-calendar"></i>
							</span>
						</div>
					...
					<div class="col-md-6">
						<div class="form-group input-group">
							<label>To::</label>
							<input type="text" data-date="" data-date-format="yyyy-mm-dd" data-link-field="any" data-link-format="yyyy-mm-dd" name="date_pickerto" id="date_pickerto" value="<?php echo isset($_POST['date_pickerto']) ? $_POST['date_pickerto'] : ''; ?>" readonly="true" class="date_pickerto form-control  input-sm">
							<span class="input-group-btn">
								<i class="fa  fa-calendar"></i>
							</span>
			...
			<form class="" method="POST" action="<?php echo $_SERVER['PHP_SELF'] ?>">
				<table class="table table-bordered table-hover" align="center" cellspacing="10px">
					...
					<tbody>
						...
						$query = "SELECT  *  FROM `tblorder` O, `tblcustomer` c,  `tblsummary` s 
							 WHERE O.`ORDEREDNUM`=S.`ORDEREDNUM` AND  c.`CUSTOMERID` = s.`CUSTOMERID`
								  AND  ORDEREDSTATS='Confirmed' GROUP BY ORDEREDDATE";
						...
						<?php
								$AMOUNT = $result->ORDEREDPRICE * $result->ORDEREDQTY;
								echo '<tr>';
								echo '<td>' . $result->ORDEREDDATE . '</td>';
								echo '<td>' . $result->ORDEREDNUM . '</td>';
								echo '<td>' . $result->FNAME . ' ' . $result->LNAME . '</td>';

								echo '<td>' . $result->ORDEREDPRICE . '</td>';
								echo '<td>' . $result->ORDEREDQTY . '</td>';
								echo '<td>' . $AMOUNT . '</td> ';
								'</tr>';

								$markupPrice += $result->ORDEREDPRICE;
								$totQty += $result->ORDEREDQTY;
								$totAmount += $AMOUNT;
							}
						} else {
							echo '<tr><td colspan="7" align="center"><h2></h2></td></tr>';
						}
						?>
						...
						<script type="text/javascript">
							...
							function drawChart() {
								...
								data.addColumn('string', 'Date Ordered');
								data.addColumn('number', 'Price Sell');

								<?php foreach ($cur as $result) { ?>
									data.addRow(['<?php echo $result->ORDEREDDATE; ?>', <?php echo $result->ORDEREDPRICE; ?>]);
								<?php } ?>

								var options = {
									title: 'Tổng doanh số theo ngày',
									hAxis: {
										title: 'Ngày đặt hàng',
										titleTextStyle: {
											color: '#333'
