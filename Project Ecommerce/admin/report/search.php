	...
	<div class="panel">
		<div class="panel-body">
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
				<span id="printout">
					<div class="row">
						<h3 align="center">Overall Income in <?php echo ((@$_POST['start']) ? $_POST['start'] : '') . ' to ' .  ((@$_POST['end']) ? $_POST['end'] : ''); ?></h3>
					</div>
				...
				FROM  `tblcustomer` c,  `tblsummary` s WHERE  c.`CUSTOMERID` = s.`CUSTOMERID` AND  ORDEREDSTATS='Confirmed' AND date(ORDEREDDATE)>='" . $_POST['start'] . "' AND date(ORDEREDDATE) <='" . $_POST['end'] . "'";
					...
					<table>
						<tfoot style="margin-right:10%">
							<tr>
								<h4 align="right"> Overall Price : &#8369 <?php echo isset($overall) ? $overall : 0; ?></h4>
							</tr>
